...
**An emerging exception — latent recursion.** A research-stage class of architectures reasons by iterating a small weight-tied block through depth rather than stacking unique layers: latent-space reasoning and looped or recurrent-depth models (COCONUT, looped transformers, the HRM/TRM line). On the compounding side this is simply another entry in the $E(t)$ ledger alongside distillation, quantisation, and sparsity. It decouples reasoning depth from parameter count, lowering the model size and therefore the cost and hardware required to clear a given sufficiency threshold (H.1). That widens the workload surface a written-down chip can serve, and strengthens the plateau case. On the binding dimension the picture is mixed rather than adverse. Pure latent recursion loops a fixed state with a flat KV cache, so its inference tilts serial-latency and compute-bound rather than memory-bandwidth-bound, toward FP8 throughput and FLOPS-per-watt, the dimensions where the crossover identity fails and newer silicon keeps its edge. But recursion will most plausibly arrive fused with sparse mixture-of-experts routing, since dense looped blocks hit representation collapse and sparse routing is the known fix. MoE decode is itself memory-bandwidth-bound, the very dimension the plateau wins on in this section; its frontier-favouring pull is VRAM capacity to hold all experts resident (the step-function of H.4), not bandwidth. A looped-MoE therefore inherits a blended binding profile, and recursion's tilt toward the frontier is partially offset by MoE's tilt back toward the plateau. Latent recursion is both ally and complication, and not yet a production scaling vector. Whether it matures into one, and which binding dimension the fused architecture finally settles on, is a forward item to watch.
...
The forward predictions remain observable on known timetables. The operator-distress trough crystallising across 2027–2028 via the three-of-six trigger gate. Secondary-market clearing prices on the pre-order pipeline as it lands — Rubin, Rubin Ultra, Feynman across 2027–2029 (H.8). The labour-market shift to verification work compounding from the 20% early-career signal of 2024–2026 (K.3). The macroeconomic productivity acceleration resolving toward one of the Goldman / Penn Wharton / Acemoglu trajectories (K.4). The convergence-of-capital effect on hyperscaler asset cascading materialising or not, observable in whether mid-market specialist-cloud and integrated-developer estates clear through Section 363 in volume at the trough. Each is checkable from public data on its own clock.
