...
The frontier also retains a class of capability that has yet to be discovered. ARC-AGI-3, the interactive reasoning benchmark released in 2026, has flatlined every frontier model — proprietary and open — at well under 1% against a 100% human baseline. The frontier is not on the verge of general intelligence. The plateau thesis does not require it to be. The plateau diffuses capability already inside the model. Whatever capability remains to be discovered is the frontier's problem; whatever is already there is the plateau's substrate.
